...
if(isset($_POST['enter'])){
    ...
    if($response->success){
        ...
        $mdp = $_POST['mdp'];
        $db = config::getConnexion();
        $stmt = $db->prepare("SELECT idclient, mdp ,verif,block,type FROM user WHERE email = ?");
        $stmt->execute([$email]);
        ...
        if ($row && password_verify($mdp, $row['mdp']) && $row['verif'] == 1 && ($row['block'] == 0)) {           
            ...
            header('location:home.php');  
            if ($row['type'] == 'Doctor') {
                header('location:/First_SUII/projet/View/homeM.php');
                 } else if ($row['type'] == 'Patient') {
                header('location:/First_SUII/projet/View/front_user/homeUM.php');
              }   
        } else if($email=="adminvitalia@gmail.com" && $mdp == "wiem") {
