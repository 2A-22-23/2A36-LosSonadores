...
if(isset($_POST['submit'])){
   ...
   $count = $stmt->fetchColumn();
   if ($count > 0) {
       $response = array('success' => false, 'message' => 'Email already exists please try with another email');
       header('Content-Type: application/json');
       echo json_encode($response);
    ...
    {    $client = new client($nom,$prenom,$type,$telephone,$adresse,$email,$login,$mdp);
        ...
        $response = array('success' => true, 'message' => 'Data inserted successfully plw now login');
        header('Content-Type: application/json');
        echo json_encode($response);
